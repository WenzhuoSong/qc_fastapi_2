...
class SynthesizerContractTest(unittest.TestCase):
    ...
    def test_normalize_preserves_position_advisory_proposals(self):
                ...
                "position_advisory_proposals": [
                    {
                        ...
                        "confidence": 1.6,
                        "reason_codes": ["llm_must_not_matter"],
                    }
                ],
            },
            base_weights={"SPY": 0.8, "CASH": 0.2},
        ...
        self.assertEqual(out["position_advisory_proposals"][0]["delta_hint"], -0.01)
        self.assertEqual(out["position_advisory_proposals"][0]["confidence"], 1.0)
        self.assertEqual(out["position_advisory_proposals"][0]["execution_authority"], "none")
        self.assertNotIn("reason_codes", out["position_advisory_proposals"][0])
        self.assertEqual(out["legacy_adjusted_weights_mode"], "diagnostic_legacy_phase_b")
        self.assertIn("diagnostic_weight_rationale", out)
