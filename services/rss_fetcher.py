...
Phase 3: HTTP fetch with retry via retry_sync for network resilience.
"""
...
def _fetch_single_feed(feed_url: str, source_name: str, max_items: int) -> List[dict]:
    ...
    try:
        feed = retry_sync(_parse, config=HTTP_RETRY_CONFIGS["rss"], service_name=f"rss_{source_name}")
    except RuntimeError:
        logger.warning(f"RSS {source_name}: all retries exhausted for {feed_url}")
        return []
