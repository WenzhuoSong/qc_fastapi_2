...
class PCPromotionConfigTests(unittest.TestCase):
    def test_default_config_is_shadow_only(self):
        ...
        self.assertTrue(cfg["require_regime_coverage"])
        self.assertEqual(cfg["min_non_bull_regime_cycles"], 2)
        self.assertEqual(cfg["min_regime_confidence_for_coverage"], 0.60)
    ...
    def test_format_mentions_auto_mode_and_no_execution_authority(self):
        ...
        self.assertIn("require_regime_coverage: True", text)
        self.assertIn("min_non_bull_regime_cycles: 2", text)
